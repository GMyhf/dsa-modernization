# 讲义与课件（第 1–12 章）

*Updated 2026-09-05 GMT+8*
 *Compiled by Hongfei Yan (2026 Fall)*
https://github.com/GMyhf/dsa-modernization

本目录存放《数据结构与算法》（张铭、王腾蛟、赵海燕，高等教育出版社 2008）
现代化重制版的**讲义（`.md`）与课件（`.pptx`）**，二者**同名成对**，
内容依据 [`../book/`](../book/) 的书稿编写。

| | 用途 | 特点 |
| --- | --- | --- |
| `*.md` 讲义 | 课后阅读、作业参考 | 完整正文、可运行代码、**习题选讲**与解答 |
| `*.pptx` 课件 | 课堂放映 | 只保留主干、关键代码与原书插图，16:9 版面 |
| `*.mp4` 视频 | 课前预习、补课、异步学习 | 课件画面 + 逐页旁白，1920×1080，带字幕文件 |

> 三者是一套东西，因此放在同一目录、同名维护：改讲义时顺手核对同名课件，
> 课件或讲稿改了就重新合成视频（闸门第 8 项会盯着这件事）。

⚠️ **讲义里的习题是选讲，不是全集。** 每章讲义收 7–21 道；
**原书全部 345 道习题与上机题的参考答案在 [`../book/习题与参考答案.md`](../book/习题与参考答案.md)**，
由 `tools/check_doc.py` 的 R14 逐题守着（只验有没有同号答案，不验答案内容）。
课件的「习题选讲」页与视频旁白都按这个口径说话。

**与 `book/slides/` 的关系**：那一套是从书稿自动抽取的放映稿，
覆盖全书 12 章、由 `tools/build_slides.py` 生成；本目录是**另起一套**、
按课堂节奏手写的讲义 + 课件，不复用它的任何内容。两者可以并存。

---

## 1 文件清单

每章一行，`.md` 与 `.pptx` 同名。

| 章 | 文件名（`.md` / `.pptx` / `.mp4` 同名） | 课件页数 | 视频时长 | 主题 |
| --- | --- | --- | --- | --- |
| 1 | `DSA_CH01_Overview_ADT_Complexity` | 47 | 29:04 | 问题求解与 Floyd、逻辑/存储结构、抽象数据类型、渐进分析 |
| 2 | `DSA_CH02_Linear_List` | 39 | 20:03 | 顺序表与链表、三法则、按位置访问 vs 改链接 |
| 3 | `DSA_CH03_Stack_Queue` | 40 | 22:43 | 栈与队列、后缀求值、运行栈与递归深度实测、循环队列 |
| 4 | `DSA_CH04_String` | 27 | 18:29 | 字符编码与串比较、串的存储、朴素匹配与 KMP |
| 5 | `DSA_CH05_Binary_Tree` | 39 | 25:10 | 递归定义与三种周游、二叉搜索树、堆与线性建堆、Huffman |
| 6 | `DSA_CH06_Tree` | 24 | 15:27 | 一般树与森林、与二叉树的等价转换、左子右兄、并查集 |
| 7 | `DSA_CH07_Graph` | 27 | 17:54 | 两种存储、DFS/BFS 与拓扑排序、Dijkstra 与 Floyd、Prim 与 Kruskal |
| 8 | `DSA_CH08_Internal_Sorting` | 31 | 21:13 | 五类排序、稳定性、实测数据、比较排序的 Ω(n log n) 下界 |
| 9 | `DSA_CH09_File_External_Sorting` | 23 | 15:38 | 成本模型换成页 I/O、置换选择、多路归并与选择树 |
| 10 | `DSA_CH10_Searching` | 26 | 20:20 | 顺序/二分/分块检索、集合与位向量、散列方法 |
| 11 | `DSA_CH11_Indexing` | 27 | 18:47 | 线性索引与多分树、倒排索引、B 树与 B+ 树、位索引 |
| 12 | `DSA_CH12_Advanced_Structures` | 28 | 21:19 | 多维数组与稀疏矩阵、广义表与存储管理、Trie、改进的 BST |

**全书 12 章已齐**，课件合计 **378 页**。章号与输出文件名的对照表在
[`build_all.py`](build_all.py) 的 `CHAPTERS` 里。

版面 16:9，中文字体**微软雅黑**，代码字体 **Consolas**。

---

## 2 源码与再生成

课件**不是手工排版的**，而是由脚本从结构化内容生成，便于批量修改样式与逐年复用。

```
courseware/
├── DSA_CHNN_*.md        # 讲义（手写维护）
├── DSA_CHNN_*.pptx      # 课件（由 build_all.py 生成）
├── DSA_CHNN_*.mp4       # 视频（由 make_video.py 合成，**不入库**）
├── deck.py              # 排版引擎：主题配色、版面构件、自适应字号
├── build_all.py         # 课件生成入口
├── make_video.py        # 视频合成入口
├── verify.py            # 闸门 9 项
├── content/
│   ├── ch01.py             # 第 1 章课件的内容（META + SLIDES）
│   └── ch01_narration.md   # 第 1 章的逐页旁白讲稿（手写维护）
├── video/
│   ├── ch01.srt            # 字幕（每页一条）
│   └── ch01.timeline.json  # 时间轴 + 课件与讲稿的 sha256
└── code/ch01/           # 讲义里上机题的参考实现（真编译、真运行）
```

⚠️ **不要直接编辑 `.pptx`** —— 它会被下次生成覆盖。改课件请改 `content/chNN.py`。
讲义 `.md` 是手写维护的，与 `content/` 无生成关系；两者内容需人工保持一致，
`verify.py` 只保证「源与产物一致」，保证不了「讲义与课件说的是同一件事」。

**环境**：`python-pptx`（读图片尺寸还要 `Pillow`）；合成视频另需
`edge-tts`、`ffmpeg`、`libreoffice`、`poppler-utils`。这是本仓库里
**唯一**允许第三方依赖的目录 —— `tools/` 与 `code/` 仍然是纯标准库。

```bash
pip install python-pptx Pillow edge-tts
```

**生成**：

```bash
cd courseware
python3 build_all.py           # 生成全部已编写的章
python3 build_all.py 01        # 只重新生成第 1 章
```

---

## 3 视频

```bash
python3 make_video.py 01                # 合成第 1 章
python3 make_video.py 01 --audio-only   # 只跑 TTS，先看总时长够不够一节课
python3 make_video.py 01 --check        # 只校验产物是否最新（闸门第 8 项）
python3 make_video.py 01 --preview      # 从成品转一份 720p 轻量版，便于传阅
```

流程：`content/chNN_narration.md` 逐页送进 edge-tts 得到旁白 → 从**当前**
`.pptx` 现导逐页 PNG（LibreOffice → PDF → `pdftoppm`）→ ffmpeg 把「一张静帧 +
一段旁白」合成一个片段 → 拼接并做响度归一化（`loudnorm I=-16`）。

因为画面是现导的，视频永远和课件同版；`video/chNN.timeline.json` 记下课件与讲稿的
sha256，任何一边改了而没重新合成，闸门第 8 项就会报「视频过期」。

**讲稿是手写的，时间控制表是机器写的。** 讲稿文末的
「附：时间控制表」由 `make_video.py` 按实测时长重写，不要手改。

`--preview` 出的是 `video/chNN-preview.mp4`：720p 单声道，第 1 章约 26 MB
（成品 48.5 MB）。很多渠道传不动 50 MB，做成脚本里的一档而不是临时敲一行
ffmpeg —— 临时命令不可复现，下次再要一份就得凭记忆重敲，参数一定会漂。
它只用来看清讲了什么，**正式放映用成品**。

**入库口径**：

| | 入不入库 | 为什么 |
| --- | --- | --- |
| 讲稿 / 字幕 / 时间轴 | ✅ | 视频的源。有它们谁都能重建出同一份 |
| `video/chNN-preview.mp4`（720p） | ✅ | 换一台机器就能直接看，不用先装 edge-tts / ffmpeg 重建一遍 |
| `DSA_CHNN_*.mp4`（1080p 成品） | ❌ | 一章约 50 MB，每次重建整个文件都变 |

成品缺席时闸门只提示、不判红 —— 和 `tools/pdfref.py` 缺扫描件时的做法一致。

> ⚠️ 轻量版一章约 26 MB，十二章就是三百多 MB，而这个仓库现在最大的已入库文件
> 才 9 MB。如果哪天嫌大，把它挪到 Git LFS 或者课程网盘，`.gitignore` 去掉
> 那条 `!video/*-preview.mp4` 即可 —— 源都在库里，随时能再生成。

⚠️ **旁白是 TTS 合成的，闸门验不了「念得对不对」。**
第 8 项只验讲稿与课件同版、页数与字幕条数一致 —— 它检查的是**结构**，不是**听感**。
数字与公式在讲稿里已写成中文读法，但**英文人名、缩写和公式仍是高风险点**
（Tony Hoare、AVL、DAG、Catalan、Patricia、McCreight……）。
**发布前请人工试听**，尤其是这些词出现的段落；哪一句别扭就改
`content/chNN_narration.md` 对应那一节再跑一次，**只有改动那一页会重新合成**。

语音默认 `zh-CN-YunyangNeural`、语速 `+0%`，用环境变量 `TTS_VOICE` / `TTS_RATE` 改。
旁白**按讲稿文本的哈希缓存**：改了哪一页就只重合成哪一页；
如果按「文件在就复用」判断，改了词却照旧用旧音频，视频和讲稿从此对不上且不会报错。

---

## 4 修改内容

编辑 `content/chNN.py` 中的 `SLIDES` 列表即可，无需碰排版代码。
每张幻灯片是一个元组：

| 写法 | 说明 |
| --- | --- |
| `('section', '第 1 节', '标题', '副标题?')` | 章节分隔页 |
| `('bullets', '标题', [条目, ...])` | 要点页；条目以 `- ` 开头表示次级 |
| `('code', '标题', '代码', '说明?')` | 代码页，字号按行数与最长行自动缩放 |
| `('ascii', '标题', '示意图', '说明?')` | 等宽示意图，居中 |
| `('image', '标题', '图片路径', '说明?')` | 原书扫描插图，等比缩放后居中 |
| `('table', '标题', [[表头...], [行...]], '说明?')` | 表格，列宽按内容自动分配 |
| `('two', '标题', '左标题', [...], '右标题', [...])` | 左右两栏 |
| `('key', '标题', '要点正文')` | 整页强调一句话 |

正文中可用 `**强调**`（渲染为深蓝加粗）与 `` `等宽` ``。
**代码页与示意图页原样输出**，不解析这些标记 —— 所以 C++ 的 `**` 与反引号是安全的。

`image` 页的路径相对 `courseware/` 解析，原书插图在
[`../book/assets/scan/`](../book/assets/scan/)。

---

## 5 质量检查

```bash
python3 verify.py            # 第 1–6、8、9 项，几秒
python3 verify.py --render   # 加第 7 项渲染检查（需 libreoffice + poppler-utils）
python3 -m unittest discover -s . -p test_verify.py -v  # 门禁回归测试
```

第 5 项逐项比较重建 PPTX 包内的 XML 与媒体字节，忽略 ZIP 时间戳和压缩方式；
文字、图片、颜色或版式变化都需要重建。第 7 项要求文字提取成功且页数与 PPTX
相符，提取失败或空输出不能视为渲染通过。

| # | 检查 | 它回答的问题 |
| --- | --- | --- |
| 1 | 配对 | 每章 `chNN.py` + `.md` + `.pptx` 齐不齐 |
| 2 | 元数据 | 讲义抬头四要素齐不齐；讲义与课件指的是不是同一章 |
| 3 | 资源 | 课件引用的插图、讲义里的相对链接，是不是都存在 |
| 4 | 代码 | `courseware/code/` 下每个 `.cpp` 在 `-Werror` 下真编译、真运行 |
| 5 | 可重生成 | `.pptx` 是不是当前 `content/` 的产物；页数与本文件的表格一致 |
| 6 | 版面标记 | 放映稿上有没有印出 `**` 或反引号；条目有没有以空白开头 |
| 7 | 渲染 | 逐页文字有没有越出版心、侵入页脚、压在一起；PDF 有没有嵌入中文字体 |
| 8 | 视频 | 讲稿是不是一页一节；时间轴记的来源摘要还对不对得上（视频过没过期） |
| 9 | 代码出处 | 讲义里每个 ```cpp 块的有效代码行，多数能不能在 `code/` 里逐行找到 |

**第 9 项是被一次过度声明逼出来的。** 讲义原本写着「每一段 C++ 都摘自代码目录，
在 `-Werror` 下编译通过并真的跑过」—— 一量才发现 32 个 ```cpp 块里只有 1 个逐字命中，
其中 4 个根本是**原书**代码的引用（编译不过，本就不该标 cpp）。
现在原书引用一律标 ```text，```cpp 由第 9 项按行核对（只比代码不比注释）。
**「全部 / 每一段 / 完整」这类全称断言，要么当场能量出来，要么就不要写。**

第 5、6 项是这套东西最容易悄悄坏掉的地方：

- **第 5 项**防的是「改了 `content/`，忘了 `build_all.py`」——
  产物与源不一致，而两个文件都还在，肉眼看不出来。
- **第 6 项**防的是排版标记漏解析。`**` 与反引号是给引擎看的，不是内容；
  漏掉一层解析它们就原样打在投影幕上。人眼对一个小小的反引号极不敏感，
  所以这件事只能由代码保证。

排版引擎自己还有一道**构建期**的保护：代码页 / 示意图页的自适应字号一旦触底
（低于 8.5pt），`deck.py` 直接抛 `LayoutOverflow` 报错，而不是钳位后照常画。
字号是构建期就能精确算出来的量，不该留给 4 分钟的渲染检查去碰运气。

> ⚠️ 字体以放映机器为准：Windows / macOS + Microsoft PowerPoint 下
> 「微软雅黑 + Consolas」可直接使用；LibreOffice 若缺少中文字体会渲染成方框，
> 闸门第 7 项会显式报出「未嵌入中文字体」，不会把这种情况当作通过。
