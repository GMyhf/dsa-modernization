...
“活动记录”是**经典教材术语**；现代资料通常称它为 **stack frame（栈帧）**，其中保存返回地址、参数和局部状态。
...
栈回答“最后进入的先出来”，队列回答“先进入的先出来”。递归调用栈就是栈 ADT 的运行时实例。
...
先把运算符按优先级放入栈，再按后缀式 `1 2 3 * +` 求值。每次弹出的顺序就是优先级规则的可视化。
让学生故意把 `+` 和 `*` 反过来，观察结果从 7 变成 9。
...
1. 读入数字进入输出队列。
2. `+` 入运算符栈，`*` 因优先级更高继续入栈。
3. 输入结束先弹 `*` 再弹 `+`，后缀式为 `1 2 3 * +`。
4. 求值为 7；忽略优先级会错误得到 9。
...
- 循环队列为什么要区分“空”和“满”？
- 递归深度很大时，如何改用显式栈？
- 看到“没有结果”时，应该返回 optional、布尔值还是抛异常？
...
- 空栈用 `empty()` 判断；循环队列用额外槽位或计数区分空和满。
- 递归深度大时，把待处理状态显式压入链式栈。
- 可预期缺失通常返回 optional；违反调用契约的越界通常抛异常。
- 队列出队要推进 front，不能只读值而不改变状态。
...
输入 `2 3 + 4 *`：读 2、3 后栈为 `[2,3]`；读 `+` 弹出 3、2 压入 5；读 4 后为 `[5,4]`；读 `*` 得 20。

---

# 反例：弹出顺序写反

减法和除法不满足交换律。栈顶是右操作数，先弹出的值必须放在右边，否则 `8 2 -` 会被错误算成 `2-8`。
