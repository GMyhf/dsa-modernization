...
![图 12.1 上三角矩阵和下三角矩阵](../assets/scan/fig-12-1.png)
...
![图 12.2 稀疏矩阵的十字链表](../assets/scan/fig-12-2.png)
...
十字链表（orthogonal list）是**历史教材表示**；工程中还常用 COO、CSR、CSC，后者连续存储、缓存局部性更好。
...
现代对应物包括嵌套对象、共享 DAG、AST 和循环对象图；“广义表”是**历史/经典教材术语**，主要用来理解这些现代结构。
...
“可利用空间表”是**经典教材术语**，即 **free list**；固定大小对象的工程实现还常称 arena、pool 或 slab allocator。
...
本章的共同问题是“哪些信息值得存、哪些关系需要共享”：稀疏矩阵压掉零元，Trie 共享前缀，引用计数和标记-清扫处理对象图。

---

# 课堂例题：同一对象被两个根引用

先画共享 DAG，再删掉一个根：引用计数只能减少计数，标记-清扫从剩余根出发仍能保留共享对象；若对象形成无根环，只有追踪式回收能收回它。
...
两个根共同引用 X 时，删除一个根只让计数减一，X 仍可达；X、Y 互相引用但不再从根可达时，引用计数仍非零。标记-清扫从根标记可达对象，未标记的环即可回收。

---

# 课末自检

- COO、CSR、CSC 和十字链表分别适合什么访问模式？
- 为什么引用计数回收不了无根环？
- Trie 的最长前缀匹配在哪个结点停止，如何记录最后一个终止词？
- AVL、伸展树和最佳 BST 分别用什么信息改善搜索代价？
...
- COO 适合构建，CSR 按行，CSC 按列，十字链表适合动态双向更新。
- 引用计数只看入边数量，环内对象互相持有引用而不会归零。
- Trie 记录最后一个终止词结点，输入结束或无边时返回它。
- AVL 用旋转，伸展树用访问局部性，最佳 BST 用访问权重动态规划。
...
根集合只含 A，A→B，B→C，X↔Y 形成无根环。标记阶段只标 A、B、C；清扫阶段回收 X、Y，即使它们互相引用。

---

# 反例：只用引用计数

X、Y 的计数都为 1，互相引用使计数永不归零；没有根可达性扫描就无法回收。
