...
![图6.3 森林转换为二叉树](../assets/scan/fig-6-3.png)
...
元素 $x$ 的等价类是 $[x]_R=\{y\in S:(x,y)\in R\}$。
所有等价类互不相交，合起来正好划分全集 $S$。
...
“带双标记”是**历史编码概念**：适合作为早期教材中的位级压缩案例，不是现代树库的主流接口。工程实现通常选择 LOUDS、平衡括号或其他 succinct tree 编码。
...
- 森林转换为二叉树时，右指针表示什么？
- 为什么按大小合并能避免树高线性增长？
- 路径压缩改变了哪些父指针，是否改变集合语义？
- 双标记表示法解决了什么空间问题，现代替代方案是什么？
...
- 左指针是第一个孩子，右指针是下一个兄弟。
- 按大小合并限制树高，路径压缩降低后续 find 代价。
- 路径压缩不改变等价类语义。
- 双标记利用空链接记录遍历信息；现代可用 LOUDS、平衡括号等编码。
