...
| 组织 | 强项 | 代价 / 今天的位置 |
| --- | --- | --- |
| 顺序 | 扫描、批处理 | 中间更新贵；顺串与列式批处理仍在用 |
| 散列 | 按键定位 | 范围扫描差；冲突和缓存按页处理 |
| 索引 | 键到记录位置 | B+ 树、LSM 等由存储引擎维护 |
| 倒排 | 属性反查记录 | 搜索系统仍以它为核心 |

机械磁带是历史例子；**顺序读写比随机交换便宜**不是。

---

# 9.2.2 文件流：概念保留，接口更新

- 文件：持久化数据对象；流：程序此刻的读写通道；缓冲与位置仍是外排序的基础。
- 原书的 `<fstream.h>`、`char*` 文件名、`int` 文件位置已经过时。
- C++17：`<fstream>`、`std::ifstream` / `std::ofstream` / `std::fstream`，文件名用 `std::string` 或 `std::filesystem::path`。
- 流对象离开作用域会关闭；要检查打开、读写与关闭错误。
- 二进制 `read` / `write` 不能直接持久化带指针或依赖字节序的 C++ 对象。

外排序算法本身不依赖某个文件 API；本书用内存模拟，避免把未经验证的 I/O 细节伪装成算法实现。
...
初始堆为 `1,4,3`，输出 1 后读入 2，继续当前顺串；输出 3 后读入 0，因 `0<3` 冻结。当前顺串为 `1,2,3,4,5`，下一顺串从冻结的 0 开始，再进行归并。
...
- 顺序读写能合并页访问，随机访问会频繁触发定位和换页。
- 置换选择把不破坏当前顺序的输入继续留在堆中。
- 选择树更新只沿一条叶到根路径，代价 O(log k)。
- 更大页和更多路数减少轮数，但增加缓冲区成本。
...
内存容量 M=3，输出 3 后读入 1；因为 `1<3`，它不能进入当前递增顺串，必须标记冻结，等下一趟重新建堆。

---

# 反例：把冻结记录立即放回堆

这样会破坏当前顺串的非递减性，归并阶段可能读到逆序记录，最终结果不再可靠。
