...
- 7.1 定义与基本术语：顶点、边、度、路径、连通
- 7.2 图的抽象数据类型：对外有哪些运算
- 7.3 存储：相邻矩阵 vs 邻接表
- 7.4 周游：DFS、BFS、拓扑排序
...
![图7.5 非连通无向图的连通分量](../assets/scan/fig-7-5.png)

- 无向图：任意两点有路径 → **连通**；极大连通子图 → **连通分量**
- 有向图：任意两点**互相**可达 → **强连通**；极大者 → **强连通分量**

![图 7.6 有向图的两个强连通分量](../assets/scan/fig-7-6.png)
...
![图 7.12 无向图的邻接表表示](../assets/scan/fig-7-12.png)
...
![图 7.8 网络实例](../assets/scan/fig-7-8.png)
...
当前 `dist[0]=0, dist[1]=3, dist[2]=9`，边 `1→2=4`；选定 1 后候选 `3+4=7`，于是更新 `dist[2]=7` 并记录前驱 1。

---

# 反例：负边

若已有 `dist[2]=2` 却存在后续负边 `1→2=-5`，先“确定”2 的贪心步骤会失效，因此 Dijkstra 禁止负权边。
