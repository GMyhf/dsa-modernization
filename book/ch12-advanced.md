...
结点所有权归容器，两条链都是裸指针，析构沿行链**迭代**释放。这里不能用 `unique_ptr` 串链——一行里非零元一多，递归析构就会压穿栈（判据见第 2.3.2a 节；实测数字在该单元的 `legacy.md` 里）。
...
标记阶段用**显式栈**而不是递归：垃圾回收恰恰是在内存吃紧时跑的，那时最不该再去吃调用栈（同 2.3.2a 的判据）。测试里有 20 万个对象的深链，递归标记必然压穿栈。
