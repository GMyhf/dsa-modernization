...
| 12.2.3–12.2.4 动态分配与无用单元回收 | 混合 | `code/ch12/storage_recovery` 示范标记-清除；真实堆管理仍为概念边界 |
...
非零元很少、分布又不规则时，叫稀疏矩阵。$m\times n$ 的矩阵里有 $t$ 个非零元，稀疏因子 $\delta=t/(mn)$；通常 $\delta<0.05$ 就按稀疏处理。这时不应再分配整块二维数组，而改存三元组 `(行, 列, 值)` 的线性表，或用十字链表：每个非零元同时挂在行链表和列链表上，便于按行、按列遍历。`code/ch12/sparse_matrix` 给出行链拥有、列链观察的现代实现。
...
引用计数收不回**环**。构造函数自底向上建表，本书的接口造不出循环表；`code/ch12/storage_recovery` 用显式根和对象图演示标记-清扫如何回收无根环。它不冒充真实运行时垃圾回收器。
