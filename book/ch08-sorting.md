...
**直接插入排序通过线性搜索来确定待插入记录的位置。** 如果前面已经有若干个记录排成了不减序列，
则对已排序记录按照从大到小依次逐个与新记录进行比较，直到找到第一个不大于新记录的值，这就是新
记录应该插入的位置；依次把新记录插入到逐步扩大的已排序子序列中，直到最后完全排好序。

对待排序序列 $R = \{r_0, r_1, \cdots, r_{n-1}\}$，插入排序按照下述步骤对 $R$ 进行排序，排完序后的
有序序列为 $R' = \{r'_0, r'_1, \cdots, r'_{n-1}\}$：(1) 首先记 $R'$ 为只含一个记录 $r_0$ 的序列，此时
$r'_0 = r_0$；(2) 将记录 $r_1$ 与序列 $R'$ 中的 $r'_0$ 进行比较，将 $r_1$ 插入到序列中适当的位置——
如果 $r_1 < r'_0$ 就将 $r_1$ 插到 $r'_0$ 前面，否则 $r_1$ 和 $r'_0$ 的相对位置保持不变；
(3) 插入 $r_i$ 时（$1 < i < n$），$\{r'_0, r'_1, \cdots, r'_{i-1}\}$ 已经有序，**将 $r_i$ 暂时保存起来、腾出该
记录所占的数组元素位置**，从右向左依次将 $r_i$ 与前面 $j$ 位置的记录 $r'_j$ 比较：如果 $r_i < r'_j$ 则
$r'_j$ 向后移动一位；如果 $r_i \ge r'_j$ 则找到了正确位置，直接将 $r_i$ 放到 $j+1$ 的位置上。最后得到
一个已经排好序的 $R'$。

具体到实现：直接插入把 `values[index]` 抽出来，向前挪动所有比它大的元素，把空位留给它。相等元素不越过彼此，
...
![图 8.5 冒泡排序](assets/scan/fig-8-5.png)
...
![图 8.7 分割过程](assets/scan/fig-8-7.png)
...
**图 8.7 那趟分割逐行读下来是这样的。** 初始序列如第一行，轴值为 32；**分割前先将轴值与最后一个
记录交换，并将轴值保存到一个临时变量中**，此时序列中的最后一个位置就是空闲位置（图中用方框表示），
$l$、$r$ 分别用来扫描整个序列的左右指针、初始时分别位于序列两端。首先 $l$ 指针从左边开始扫描，
找到 34 大于轴值就停下；**将这个逆置记录放到空闲位置 $r$ 上**，这时 34 原来所在位置 $l$ 就空闲了、
可以用来存放下一个逆置记录；然后从 $r$ 前面的记录开始寻找小于轴值的逆置记录，找到 29 停下，
**将 29 放到空闲位置 $l$ 上**，这时 29 原来所在位置 $r$ 变为空闲。不断循环下去，直到 $l$、$r$ 相遇，
表示整个序列已经扫描完毕；**此时的空闲位置正好就是轴值的正确位置，将轴值 32 回填回去**——
可以看到轴值左边的记录都小于它、右边的记录都大于它，分割过程完毕。

**等于轴值的记录怎么办？** 由于快速排序是不稳定的，因此等于轴值的情况可以有多种处理方法。原书算法8.6
里那句 `while (Array[l] <= TempRecord && r > l) l++;` 的注释写着「`<=` 也可以改写为
`<`，但增加记录移动」——**两种写法都对，差别只在移动次数上。**
...
![图 8.9 桶式排序示意图](assets/scan/fig-8-9.png)
...
当 $m$ 为 $\Theta(n)$ 数量级时，时间代价为 $\Theta(n)$——此时桶式排序的效率
...
**先把记号定下来。** 假设一个长度为 $n$ 的序列 $R = \{r_0, r_1, \cdots, r_{n-1}\}$，记录的排序码 $K$ 包含
$d$ 个子排序码 $K = (k_{d-1}, k_{d-2}, \cdots, k_1, k_0)$，称序列 $R$ 对排序码有序，就是对于任意的两个
记录 $R_i$、$R_j$（$0 \le i < j \le n-1$）都满足
$(k_{i,d-1}, \cdots, k_{i,0}) \le (k_{j,d-1}, \cdots, k_{j,0})$；其中 $k_{d-1}$ 称为最高位排序码，
$k_0$ 称为最低位排序码。

**这 $d$ 个排序码可以是不同类型。** 例如按照桥牌规则，扑克牌的排序码就是（花色，面值）的组合值：
花色按黑桃 > 红心 > 梅花 > 方片的顺序，面值按 A > K > Q > J > 10 > 9 > $\cdots$ > 3 > 2 的顺序。
人们对整数的运算是十进制的，因此本章例子都选取 10 作为基数；其实对于由 0、1 代码组成的整数，
还可以看做二进制、四进制、八进制、十六进制，而采用基数 $r = 2, 4, 8, 16$；**对于字符串，则可以采用
基数 $r = 26$（英文字母个数）。**

**按照排序码的先后顺序，分配排序分为两种。** **高位优先法**（most significant digit first，MSD）：
先对高位 $k_{d-1}$ 排序，将序列分成若干子序列，每个子序列具有相同 $k_{d-1}$ 值；然后对每个子序列按
次高位 $k_{d-2}$ 排序，分成更小的子序列；依次重复，直到对 $k_1$ 排序后，分成的极小子序列每个内部
含有相同的排序码；最后对每个子序列用最低位 $k_0$ 排序，将所有子序列依次连接在一起，成为一个有序
序列——**这是一个分、分、$\cdots$、分、收的过程，是一个递归分治问题。**
**低位优先法**（least significant digit first，LSD）：从最低位 $k_0$ 开始排序，对于排好的序列再用次低位
$k_1$ 排序，依次重复，直至对最高位 $k_{d-1}$ 排好序后整个序列成为有序的——**这是一个分、收；分、收；
$\cdots$；分、收的过程。**

以扑克牌 `S3 HJ C8 H9 S9 D3 CA D7` 为例：**MSD 方法**先按花色分 4 堆、各堆中再按牌点分，最后把
4 堆合并（先按花色得 `C8 CA D3 D7 HJ H9 S3 S9`，再按面值得 `C8 CA D3 D7 H9 HJ S3 S9`）；
**LSD 方法**先按牌点分成顺序的 13 堆，再依次从这 13 堆中顺序取出 4 种花色（先按面值得
`S3 D3 D7 C8 H9 S9 HJ CA`，再按花色得 `C8 CA D3 D7 H9 HJ S3 S9`）。

**在人们看起来似乎觉得 MSD 比较直观，一般都采用 MSD 方法；但是计算机常用 LSD 进行分配排序，
因为它速度较快、便于统一处理**——而 MSD 在分配之后还要处理各子集的排序问题，这通常是一个递归
过程，计算机处理比较复杂。**本节的算法都是讨论 LSD 方法。**

具体地说，基数排序把关键码拆成 $d$ 位，每一位的取值只有 $r$ 种（十进制数 $r=10$，字符串 $r=26$），
于是可以逐位做桶式排序。

![图 8.10 基数排序（低位优先）](assets/scan/fig-8-10.png)
...
![图 8.11 基于顺序存储和桶排序的基数排序](assets/scan/fig-8-11.png)
...
![图 8.12(b) 第一趟分配结果：按个位把 9 个关键码挂到 10 条静态链上](assets/scan/fig-8-12-b.png)
...
![图 8.12(d) 第二趟分配结果：按十位再分配一次](assets/scan/fig-8-12-d.png)
...
![图 8.13 索引排序示意图](assets/scan/fig-8-13.png)
...
直接插入排序、冒泡排序等算法的平均时间代价是 $\Theta(n^2)$，堆排序、快速排序、
...
4. **堆排序**：对所有记录建立最大堆（也可以用最小堆），取出堆顶的最大记录，重新建堆；不断重复这一
   操作，直到堆为空，得到完整的有序序列。
5. **冒泡排序**：依次比较相邻的两个记录，若逆序则交换，每趟把当前无序区中最大的记录
   "冒泡"到本趟的最末端。反复进行直到某一趟没有发生交换为止。
...
> 以下是原书第 8 章的习题（原书共 33 道，此处按扫描件补回题面）；**参考答案见 `book/习题与参考答案.md` 的同章小节**，
