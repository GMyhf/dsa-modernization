...
排序把一个序列重排为非递减顺序。阅读时同时问四个问题：比较还是分配？是否稳定？额外空间多少？
最好、平均、最坏时间是多少？插入和冒泡适合近乎有序的数据；堆和快速排序适合通用场景；归并
稳定但需要辅助空间；计数和基数排序利用整数键的结构，而非只比较大小。

源码入口：[全部手写排序](../code/ch08/sorting/modern.hpp)、
[对拍测试](../code/ch08/sorting/test.cpp)。运行：
`python3 tools/check_code.py --allow-degraded code/ch08/sorting`。
...
“稳定”指相等键在排序前后的相对顺序不变。例如记录按成绩排序时，两名同分学生仍按原来的姓名
顺序出现。若排序只是整数，稳定性常不可见；若键和记录绑定，它会影响多关键字排序。
