...
![图 6.2 树形结构的凹入表、文氏图和嵌套括号表示](assets/scan/fig-6-2.png)
...
![图 6.3 森林经过连线、切线、旋转转换为二叉树](assets/scan/fig-6-3.png)
...
![图 6.4 二叉树转换为森林](assets/scan/fig-6-4.png)

图 6.4　上述转换的逆过程：(a) 二叉树；(b) 它对应的森林。

![图 6.5 森林和对应的二叉树](assets/scan/fig-6-5.png)

图 6.5　森林与对应二叉树的互相转换：(a) 森林；(b) 森林对应的二叉树。
...
![图 6.6 以“子结点表”表示法实现图 6.1 中的树](assets/scan/fig-6-6.png)
...
![图 6.7 “左子/右兄”表示两棵树](assets/scan/fig-6-7.png)
...
![图 6.8 使用静态“左子/右兄”实现对树的归并](assets/scan/fig-6-8.png)

图 6.8　静态数组中的左子、右兄链接和树的归并。

### 6.2.3 动态表示法

**问题的根源在于「度不固定」。** 树中的结点可以有任意的度数，并且各结点的子结点数随时可能发生
...
![图 6.9 树的动态表示法：(a) 树；(b) 树的实现](assets/scan/fig-6-9.png)
...
![图 6.11 集合的表示方法：(a) 子集 $S_1$；(b) 子集 $S_2$；(c) 并集 $S_3$](assets/scan/fig-6-11.png)
...
![图 6.12 合并操作的一个极端情况：(a) $n$ 个集合；(b) 合并操作](assets/scan/fig-6-12.png)

图 6.12　不采用重量权衡时，反复合并可能形成极端退化的树：(a) 起初是 $n$ 个单元素集合，(b) 每次都把大树挂到小树下，最后退化成一条长链。

**第二条是路径压缩**：`find` 在返回根之前，把沿途每个结点的父指针都直接改成根。

![图 6.13 路径压缩示例：(a) 路径压缩之前；(b) 路径压缩之后](assets/scan/fig-6-13.png)
...
![图 6.14 带右链的先根次序表示法](assets/scan/fig-6-14.png)
...
以原书图6.5(a) 那片森林为例，它的双标记先根次序表示（图 6.15）是：

![图 6.15 带双标记的先根次序表示法](assets/scan/fig-6-15.png)
...
> **照抄这张图很容易把两行读反。** 图 6.15 的三行从上到下是 `rtag` / `info` / `ltag`——
> `ltag` 在**最下面**，不在最上面。按定义核一遍就知道哪行是哪行：`B` 在图 6.5(a) 里是叶结点，
> 它的 `ltag` 必须是 1；`B` 有兄弟 `C`，它的 `rtag` 必须是 0。
> 两行一旦互换，还原出来的是**另一片森林**（`C` 会变成 `B` 的孩子而不是兄弟），
> 而它的**先根序列恰好还是 `A B C E F D G H J I`**——所以只对先根序列是查不出来的，
> 得同时对后根序列（`B E F C D A J H I G`）。本书的测试两条都对。
...
![图 6.16 带度数的后根次序表示法](assets/scan/fig-6-16.png)

图 6.16　结点度数随后根次序一起保存。

### 6.3.4 带双标记的层次次序表示

![图 6.17 带双标记的层次次序表示法](assets/scan/fig-6-17.png)
...
**对于树的概念，本书给出了两种等价的定义方式**：(1) 从递归来看，树可以看做是由根结点和子树构成，
而每一棵子树也是树；(2) 从二元组的逻辑结构来看，树是结点的有穷集合以及在该集合上定义的一个
二元关系。在树的基本概念中，还给出了树及森林的相关概念，例如树叶、根结点、边、层数、度数、有序树
等——**这些概念基本上沿用二叉树一章中的相关术语**。

**树与二叉树、森林与二叉树之间存在一一对应的关系**，本章介绍了它们相互转化的方法。

树和森林的周游是十分重要的运算。按深度优先周游树和森林包括先根次序和后根次序周游：
**按先根次序周游森林等同于按前序法周游树或森林对应的二叉树，按后根次序周游森林等同于按中序法
周游对应的二叉树。** 广度优先周游树，是指从树的第零层（根结点）开始，自上至下逐层周游；在同一层
中，按照从左到右的顺序对结点逐一进行访问，直到访问完最下一层的所有结点。

**本章重点介绍了树的存储结构**，联系到具体的应用可以选择不同的存储方式。常用的链式存储结构有
「子结点表」、静态「左子/右兄」、动态表示法、「左子/右兄」二叉链表表示法和父指针表示法等：
**「左子/右兄」二叉链表是采用得最多的表示法**；父指针表示法可以方便地实现并查集，常用于维护由
一些不相交子集构成的集合，判断两个结点是否在同一个集合中以及归并两个集合（路径压缩与重量权衡
合并让大量操作接近常数）。

顺序存储表示法把树中结点按照一定的顺序存储到一片连续的存储单元中。**「带双标记的先根次序表示」、
「带度数的后根次序表示」和「带双标记的层次次序表示」都是与树/森林遍历次序相关的顺序存储结构**，
根据存储方法的性质加入了必要的附加信息。

在本章的最后还介绍了 $K$ 叉树的概念及性质：$K$ 叉树是由一个根结点及 $K$ 棵互不相交的 $K$ 叉树
构成；二叉树的许多性质可以推广到 $K$ 叉树，通常用数组实现完全 $K$ 叉树。
...
> 以下是原书第 6 章的习题，本轮按扫描件补回题面；**参考答案见 `book/习题与参考答案.md` 的同章小节**，
> 由 `tools/check_doc.py` 的 R14 逐题守着。

1. 给出一棵树的逻辑结构 $T = (K, R)$，其中
   $K = \{A, B, C, D, E, F, G, H, I, J, K, L, M\}$，
   $R = \{\langle A,B \rangle, \langle A,C \rangle, \langle A,D \rangle, \langle B,E \rangle, \langle B,F \rangle,
   \langle B,G \rangle, \langle C,H \rangle, \langle D,I \rangle, \langle D,J \rangle, \langle F,K \rangle,
   \langle F,L \rangle, \langle I,J \rangle\}$。
   试用树形表示法画出此树，并按照递归定义将树划分为子树，指出哪些结点是根、哪些结点是树叶，
   确定各结点的层数、度数，并将该树转化为对应的二叉树。
2. 3 个结点可以构造出多少个不同的有序树？试将其画出来。
3. 将图 6.19 中的二叉树转换为对应的森林，分别按照先根次序、后根次序列出其结点。

![图 6.19 习题 3 的图示](assets/scan/fig-6-19.png)

图 6.19　习题 3 的图示

4. 编写出一个算法来判断两棵树是否相同。**尽可能提高算法效率**，并分析算法的运行时间代价。
5. 编写出一个算法来判断两棵树在**不考虑子树顺序**的前提下是否相等。尽可能提高算法效率，
   分析算法的运行时间代价。
6. 编写一个函数，以一棵树为输入，返回树的结点数目。
7. 分析「子结点表」表示法、「左子/右兄」二叉链表表示法的**结构性开销比率**。相比之下它们各自的
   空间效率如何？
8. 使用重量权衡合并规则与路径压缩，对下列从 $0 \sim 15$ 之间的数的等价对进行归并，并给出所得树的
   父指针表示法的数组表示。在初始情况下，集合中的每个元素分别在独立的等价类中；当两棵树规模
   同样大时，使结点值较大的根结点作为值较小的根结点的子结点。
   $(0,2)\;(1,2)\;(3,4)\;(3,1)\;(3,5)\;(9,11)\;(12,14)\;(3,9)\;(4,14)\;(6,7)\;(8,10)\;(8,7)\;(7,0)\;(10,15)\;(10,13)$
9. 编写函数，以一棵树的根为输入，返回一棵对应的二叉树。
10. 一个高度为 $L$ 的 $K$ 叉树，如果按层次顺序对树中所有结点从 1 开始编号，问：(1) 编号为 $N$ 的
    结点的父结点（若存在）编号是多少？(2) 编号为 $N$ 的结点的第 $i$ 个子结点（若存在）编号是多少？
    (3) 编号为 $N$ 的结点有右兄弟的条件是什么，其编号是多少？
11. 画出图 6.20 所示二叉树对应森林的左子/右兄法存储表示、带右链的先根次序存储表示、带左链的
    层次次序存储表示和带度数的后根次序存储表示。

![图 6.20 习题 11 的图示](assets/scan/fig-6-20.png)
...
1. 使用带子结点指针数组的树的链接表示法，实现 6.1.3 节中定义的树类。**此实现必须能支持结点的
   子结点数目的变化**：在生成时，结点应该只被分配只够存储其初始子结点集的空间；当一个新的子结点
   添加到结点上以致于数组溢出时，则从可利用空间中分配一个为原来数组两倍大的数组。
2. 使用父指针表示法，实现 6.1.3 节中定义的树类。
3. 若用大写字母标识树的结点，则可用**带标号的广义表形式**表示一棵树，其语法为
   「树 → 大写字母 [ `(` 树 { `,` 树 } `)` ]」（图 6.21）。编写递归算法：(1) 将这种表示方法的字符序列
   转化成树的左子/右兄表示法；(2) 将用左子/右兄表示法表示的树转化为这种表示法的字符序列。

![图 6.21 上机题 3 的图示](assets/scan/fig-6-21.png)

图 6.21　上机题 3 的图示：广义表形式的语法图

4. 另一种表示二维数组的方法是 **4 叉树表示法**，其中一个结点的 4 个儿子分别表示数组的
   NE、SE、SW、NW 4 个分块（图 6.22）。这种表示二维数组的 4 叉树称为**金字塔**。

![图 6.22 上机题 4 的图示](assets/scan/fig-6-22.png)

图 6.22　上机题 4 的图示：一个结点的 4 个子结点对应数组的 4 个分块

   (1) 设计一个基于金字塔表示法的矩阵乘法算法；(2) 对于给定的 $2^p \times 2^p$ 矩阵，设计一个构造
   表示这个矩阵的二叉树算法，并将这个算法推广到可表示任意 $M \times N$ 矩阵的情形，算法效率如何？
