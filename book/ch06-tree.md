...
一般树允许一个结点有任意多个孩子。本章用“左孩子、右兄弟”表示它：`child` 指向第一个孩子，
`sibling` 指向下一个兄弟。先根周游先访问父结点，后根周游先访问全部孩子，层次周游按离根的
距离逐层访问。并查集维护若干互不相交的集合，用于判断两个元素是否已经连通。

源码入口：[一般树和并查集](../code/ch06/general_tree/modern.hpp)、
[测试](../code/ch06/general_tree/test.cpp)。运行：
`python3 tools/check_code.py --allow-degraded code/ch06/general_tree`。

### 孩子-兄弟表示法

例如 A 的孩子是 B、C、D，B 的孩子是 E、F。A 的 `child` 指向 B，B 的 `sibling` 指向 C，
C 的 `sibling` 指向 D；B 的 `child` 指向 E，E 的 `sibling` 指向 F。这样，一个任意度树只需
两个指针域就能保存。代价是不能 O(1) 直接取得“第 k 个孩子”，必须沿兄弟链走过去。

三种周游的访问顺序不同：先根是“结点、孩子子树”；后根是“孩子子树、结点”；层次周游则先
访问根，再访问根的所有孩子，再访问下一层。递归版本最贴合树的定义；层次周游需要队列。

### 并查集为什么快

并查集只回答两类问题：元素属于哪个集合，以及两个元素是否同集合。`find(x)` 沿父指针走到根；
路径压缩让沿途结点以后直接指向根。`unite(a, b)` 把两个根合并，并优先把较矮的树挂在较高的
树下。两项优化一起使大量操作的平均代价接近常数。
