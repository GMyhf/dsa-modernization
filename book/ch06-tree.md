...
递归周游与销毁在极深树上有栈溢出风险，见 [`collab/UNVERIFIED-RISKS.md`](../collab/UNVERIFIED-RISKS.md)。
...
`create_root` 清空旧树并新建根。`insert_first` 先让新结点的 `sibling` 指向原来的第一个孩子，再把它接到 `parent->child` 上——所以后插入的孩子会出现在兄弟链前端。

`delete_subtree` 不能只 `delete` 那个结点：它可能是根的孩子，也可能是某个兄弟。实现沿着「父的孩子链」或「森林的根兄弟链」找到指向它的指针，改写成指向它的下一个兄弟，再递归销毁整棵子树。

先根周游对当前层的每个兄弟：先访问自己，再递归进入孩子。后根则先递归孩子，再访问自己。层次周游用一个数组当队列，先把森林的全部根入队，再按入队顺序扩展孩子。

并查集的 `find` 在返回根之前把沿途结点的父指针改成根，这就是路径压缩。`unite` 比较两棵树的秩，把较矮的挂到较高的下面；两棵一样高时，新根的秩加 1。
