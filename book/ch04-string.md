...
连续存储使按下标取第 $i$ 个字符为 $O(1)$，但在中间插入或删除时必须移动其后的字符，拼接和抽取子串也要复制一段连续区域。若容量不足，通常重新申请更大的数组、复制旧内容、释放旧数组，再更新指针和长度；这也是后面 `String` 类需要自己维护所有权、拷贝和析构的原因。
...
C 标准库在这块内存上定位字符：`strchr(s1, 'o')` 找第一次出现，`strrchr(s1, 'o')` 找倒数第一次出现。

![图4.2 在字符串 s1 中寻找并定位给定字符 'o' 的位置](assets/3fd8da33af3abe6c.jpg)
