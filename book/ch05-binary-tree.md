...
![图 5.1 二叉树的 5 种基本形态中的 (c)(d)(e)](assets/combined/fig-5-1.png)
...
![图5.2 满二叉树和完全二叉树示例](assets/combined/fig-5-2.png)
...
链式存储把结点散放在内存里，结点之间的关系用指针表示。每个结点除了数据域 `info`，还有 `left`、`right` 两根指针；孩子为空时对应指针为空。
...
完全二叉树还有一种更省的存法：按层次从上到下、从左到右把结点编号 $0 \ldots n-1$，编号 $i$ 的结点就放在数组下标 $i$。
...
二叉搜索树要求左子树的键都小于根、右子树都大于根。中序周游因此正好是排序。原书用关键码集合 $K=\{50,19,35,55,20,5,100,52,88,53,92\}$ 建的树是这样的：
...
插入重复键、删除不存在的键都是可预期状态，返回 `false`，不抛异常。删除有左右孩子的结点时，用左子树里最右的前驱替换它：先把前驱从原位置摘下，再让它继承被删结点的两棵子树，最后只 `delete` 被删结点一次。漏掉「先脱离原父」会形成环或二次释放。
...
![图 5.12 二叉搜索树的删除示例](assets/combined/fig-5-12.png)
...
![图5.17 建堆过程示例](assets/d7354d550a2f5d79.jpg)
...
![图 5.18 具有不同带权外部路径长度的二叉树](assets/combined/fig-5-18.png)
...
![图 5.19 Huffman 树的构造过程](assets/combined/fig-5-19.png)
