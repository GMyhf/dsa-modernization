...
顺序栈和链式栈在本章承担的是存储管理课：数组容量如何扩展、元素如何构造和销毁、链式结点由谁拥有，以及扩容失败时如何保持原栈不变。Python `list.append` 会把这些决定全部藏起来，手写一个 Python 栈也不会让三法则、五法则或强异常保证变得可观察，所以本节不提供“把 C++ 逐行翻译成 Python”的版本。
