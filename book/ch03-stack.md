...
所有元素前移一位，O(n)；反过来把队尾固定在末尾，入队 O(1) 而出队要搬 n−1 个元素。
两头都别固定，让 `front` 和 `rear` 都在数组里往后走，两个操作就都是 O(1)：
...
顺序栈还有一个链式栈没有的用法：**两个栈共享一个数组**。编译系统这类同时管着多个栈的场合会用到它——把数组两端分别当作两个栈的栈底，各自向中间迎面增长：
