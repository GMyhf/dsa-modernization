...
![图 7.1 用图描述通信网络](assets/scan/fig-7-1.png)
...
![图 7.2 图的示例](assets/scan/fig-7-2.png)
...
![图 7.3 完全图](assets/scan/fig-7-3.png)
...
![图 7.4 图 7.2 的若干子图](assets/scan/fig-7-4.png)
...
![图 7.5 非连通无向图的连通分量](assets/scan/fig-7-5.png)

图 7.5　(a) 非连通的无向图 $G_3$；(b) $G_3$ 的两个连通分量。「极大」是指再添任何一个顶点，子图就不连通了。

![图 7.6 有向图 $G_2$ 的两个强连通分量](assets/scan/fig-7-6.png)
...
![图 7.7 图 7.2 中无向图和有向图的生成树](assets/scan/fig-7-7.png)
...
![图 7.12 无向图 $G_1$ 的邻接表表示](assets/scan/fig-7-12.png)
...
![图 7.13 有向图 $G_2$ 的邻接表和逆邻接表表示](assets/scan/fig-7-13.png)

图 7.13　有向图 $G_2$ 的 (a) 邻接表与 (b) 逆邻接表。逆邻接表里 $v_i$ 的边表长度就是它的入度——7.4.3 节拓扑排序要的正是入度。

带权图在边表结点里多存一个权：

![图 7.14 带权图 $G_4$ 的邻接表表示](assets/scan/fig-7-14.png)
...
![图 7.15 有向图的十字链表示例](assets/scan/fig-7-15.png)
...
![图 7.16 有向图 G](assets/scan/fig-7-16.png)
...
![图 7.17 表示课程优先关系的有向无环图](assets/scan/fig-7-17.png)
...
![图 7.18 最短路径的示例](assets/scan/fig-7-18.png)
...
![图 7.19 单源最短路径的示例](assets/scan/fig-7-19.png)
...
![图 7.20 每对顶点间的最短路径的示例](assets/scan/fig-7-20.png)
...
![图 7.22 含 (u,v) 的回路](assets/scan/fig-7-22.png)
...
![图 7.23 带权图](assets/scan/fig-7-23.png)
...
![图 7.24(a)(b)(c) Prim 算法构造最小生成树的前三步](assets/scan/fig-7-24-abc.png)

![图 7.24(d)(e)(f) Prim 算法构造最小生成树的后三步](assets/scan/fig-7-24-def.png)

图 7.24　Prim 算法在图 7.23 上的六个步骤（原书印在相邻两页上，(a)(b)(c) 一页、(d)(e)(f) 一页，这里照原样分作两张）。注意每一步长出来的都是**一棵连着的树**——这正是它与 Kruskal 的区别。
...
![图 7.25(a)(b)(c) Kruskal 算法构造最小生成树的前三步](assets/scan/fig-7-25-abc.png)

![图 7.25(d)(e)(f) Kruskal 算法构造最小生成树的后三步](assets/scan/fig-7-25-def.png)

图 7.25　Kruskal 算法在同一张图 7.23 上的步骤（同样跨两页，(a)(b)(c) 与 (d)(e)(f) 各一张）。中间过程是**一片森林**，直到最后才连成一棵树；判断「这条边会不会成环」就是判断两个端点是不是已经在同一棵树里——这正是第 6 章并查集的用处。两个算法走的路不同，得到的总权相同。
...
> 以下是原书第 7 章的习题，本轮按扫描件补回题面；**参考答案见 `book/习题与参考答案.md` 的同章小节**，
> 由 `tools/check_doc.py` 的 R14 逐题守着。题面里引用的图，一并从扫描件裁了回来。
...
![图 7.26 习题 1 的图例](assets/scan/fig-7-26.png)
...
![图 7.27 习题 2 的图例](assets/scan/fig-7-27.png)

图 7.27　习题 2 的图例

3. 求图 7.28 所示的有向图中从顶点 $v_4$ 到其他各顶点的全部最短路径及长度。

![图 7.28 习题 3 的图例](assets/scan/fig-7-28.png)
...
![图 7.29 习题 4 的图例](assets/scan/fig-7-29.png)
