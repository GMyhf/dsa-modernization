...
图 1.4 索引：主表按记录存放，旁侧一张按关键码排序的表指向记录位置。

```text
主文件（无序）              线性索引（按 key 排序）
┌────┬────────┐            ┌────┬────┐
│ #  │ 记录   │            │key │位置│
├────┼────────┤            ├────┼────┤
│ 0  │ 张三   │◄───────────│ 12 │ 2  │
...
ADT 是三元组 (D, R, P)：数据对象、对象上的关系、一组运算。对外只看见运算，看不见存储。原书【代码1.2】用一个空模板类描述这件事；本书各章把运算直接写在实现类上。空基类给不了多态，还会带来非虚析构。`new T[n]` 会默认构造整块槽位，所以元素类型必须能默认构造——这句话写在正文里即可。
...
图 1.5 常用阶的相对增长（同一 n 下后一行远大于前一行）：
