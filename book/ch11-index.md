...
![图11.1 线性索引与不等长记录](assets/scan/fig-11-1.png)
...
![图 11.5 3 阶 B 树](assets/scan/fig-11-5.png)
...
**检索要算访外次数。** 在含 $n$ 个关键码的 $m$ 阶 $B$ 树上检索，从根结点到关键码所在结点的路径上
涉及的结点数不超过 $h+1$，即最多 $h+1$ 次访外。以原书图 11.5 为例：检索关键码 31 需要 3 次访问
索引树结点（$a$、$c$、$i$），外加 1 次读关键码 31 所在主文件的数据盘 $a_{12}$，**共 4 次访外**；
而检索关键码 12 只需要 2 次访问索引树结点（$a$、$b$），外加 1 次读数据盘 $a_3$，**共 3 次访外**。

**插入的完整规则。** 在 $B$ 树中插入一个关键码的方法也很简单，**插入可能导致 $B$ 树朝着根的方向
生长**：首先检索此关键码是否在 $B$ 树中，若在则拒绝插入；若不在，则最后找到的叶结点即为待插入位置。
如果插入后该叶结点中关键码个数小于 $m$，则插入完成；否则关键码个数等于 $m$，**产生上溢出**——
这种情况下，取这 $m$ 个关键码的中位数为分界码，叶结点分裂为分别具有 $\lceil (m-1)/2 \rceil$ 和
$\lfloor (m-1)/2 \rfloor$ 个关键码的两个结点，分界码插入到父结点中；如果父结点没有发生上溢出，插入
操作结束，**否则该分裂过程继续往根的方向传递，有可能一直分裂到根**。

**插入的访外次数可以算出来**：找插入结点要向下读盘 $h$ 次；每分裂一个非根结点要向磁盘写出 2 个结点，
分裂根结点（最后一次）要写出 3 个结点，因此最坏情况下

$$\text{插入操作最多访外次数} = h + 2(h-1) + 3 = 3h + 1$$

按这个公式，原书图 11.7 那次根也产生分裂的插入，访外为 $3\times3+1 = 10$ 次。

**根结点为什么可以只有 2 个子结点？** 根结点允许的子结点数目范围在 $2 \sim m$ 之间，而内部结点的
子结点数目在 $\lceil m/2 \rceil \sim m$ 之间——**这是因为插入算法分裂到根结点时会分为 2 个子结点并
产生一个新根**；如果要求与其他内部结点具有同样的出度，就需要对 $B$ 树进行比较大的调整，这就违背了
$B$ 树操作的局部调整原则。而且实际运行时，**根结点往往在内存中保存**（当然最终它还是要写回外存，
因此也有 $m$ 的上限），不需要遵守内部结点的子结点数目下限约束；大部分内部结点在外存中，需要保证
存储块至少半满以折衷存储效率和操作效率，因此限制在 $\lceil m/2 \rceil \sim m$。**根结点也不可能只有
一个子结点，那样太浪费存储空间，而且会相应地增加检索时间。**

**为什么插入时分裂而不是「送给邻居一些关键码」？** 这是由于插入操作可能发生在创建阶段，分裂的结点
很快就会增加新的关键码；**而送关键码涉及读和修改邻居结点，访外开销大，且操作麻烦。**

插入使结点超过上限（溢出）时，把它分裂成两个，并把分界 key 上推到父结点；父结点也可能接着分裂，最坏会一直裂到根，树增高一层。

![图11.6 在图 11.5 的 3 阶 B 树中插入关键码 14、55 后的结果](assets/scan/fig-11-6.png)

图 11.6　在图 11.5 的 3 阶 B 树里插入 14、55。两次插入都能就地放下，树高不变——**大多数插入就是这样结束的**。

![图11.7 在图11.5 的 3 阶 B 树中插入关键码 19 后导致根分裂](assets/scan/fig-11-7.png)

图 11.7　再插入 19 就不行了：叶溢出、分裂、分界 key 上推，父结点跟着溢出，一路裂到根，树**增高一层**。B 树只在这一种情况下长高，所以它的所有叶永远在同一层。

**删除的完整规则。** 删除的过程与插入类似，但比插入稍微复杂些。**如果删除的关键码不在叶结点层，
则需要先把此关键码与其在 $B$ 树中的后继对换位置，然后再删除该关键码**——在 $B$ 树中，一个关键码的
后继就是比它大的那些关键码中值最小的那一个，非叶层待删关键码的后继关键码一定处于叶层，即待删
关键码的右指针所指子树中最左结点的最小关键码。如果删除的关键码在叶结点层，则直接从叶结点中删除，
**这可能导致此结点所包含的关键码的个数小于 $\lceil m/2 \rceil - 1$，产生下溢出。**

这种情况下，考察该结点的直接左兄弟或右兄弟（**习惯上先左后右**），从兄弟结点借若干个关键码到该
结点中。实际上，**往往是被删除结点和被借结点的关键码、还有父结点中的分界码一起排列，取中位数为
新分界码**，尽量使得两个新兄弟结点所含关键码个数基本相同（这样可以延迟下一次删除引起的下溢出）。

当在兄弟结点的关键码个数也很少、刚好等于 $\lceil m/2 \rceil - 1$ 时，不能借关键码。这种情况下，
**要把删除关键码的结点、它的左/右兄弟（先左后右）结点及父结点中的分界关键码合并为一个结点**；
合并的情况导致父结点中减少了一个关键码，这样有可能导致进一步的合并。**合并如果一直传递到根结点，
将导致根结点与两个子结点进行合并，形成新的根结点，从而使整个树减少了一层。**

删除过程中，一般情况下出现下溢出时先看是否能借关键码，借不了再考虑合并。**如果阶比较小**，例如
阶为 3（关键码数在 1～2 个）、阶为 4（关键码数在 1～3 个），此时删除可能会出现空结点的情况，
**可以先考虑是否能合并，再考虑借关键码，这样有可能降低树的高度**；实际应用中 $B$ 树的阶很大，不会
出现删除一个关键码使得结点变空的情况。

删除使结点低于下限（下溢）时，先向左右兄弟借 key；借不到就把两个结点合并，分界 key 从父结点拿下来。借和合并都可能向上传播。

![图11.8 5 阶 B 树，连续删除关键码 120、150](assets/scan/fig-11-8.png)
...
![图 11.10 插入后 B+ 树增高一层](assets/scan/fig-11-10.png)
...
![图 11.11 从 3 阶 B+ 树删除 75 后](assets/scan/fig-11-11.png)
...
![图11.12 混合型 B+ 树，内部结点阶为 4，叶结点阶 5](assets/scan/fig-11-12.png)

图 11.12　混合型 B+ 树：内部结点阶为 4，叶结点阶为 5。

![图 11.13 在图11.12 混合型 B+ 树中插入 22 后的结果](assets/scan/fig-11-13.png)

图 11.13　在图 11.12 的混合型 B+ 树里插入 22 之后。

![图11.14 在图11.13 混合型 B+ 树中删除 40 后的结果](assets/scan/fig-11-14.png)
...
![图 11.15(a) 百货销售数据库记录以及 State=NY（纽约州）的位向量](assets/scan/fig-11-15-a.png)
...
![图 11.15(c) 数据域 Class 的位向量集合](assets/scan/fig-11-15-c.png)
...
![图 11.16 红黑树示意图](assets/scan/fig-11-16.png)
...
![图11.17 父结点是红色、叔父结点是黑色的情况](assets/scan/fig-11-17.png)

图 11.17　父结点红、叔父黑：旋转一次解决红红冲突。图中新增结点 X 是父结点 A 的左孩子；X 是右孩子时先把 X 与 A 换位，就化归成同一种形状。

![图11.18 叔父结点为黑色时的四种重构](assets/scan/fig-11-18.png)
...
![图11.19 父结点、叔父结点均为红色](assets/scan/fig-11-19.png)
...
![图 11.20 红黑树插入示例](assets/scan/fig-11-20.png)
...
> 以下是原书第 11 章的习题，本轮按扫描件补回题面；**参考答案见 `book/习题与参考答案.md` 的同章小节**，
> 由 `tools/check_doc.py` 的 R14 逐题守着。图 11.1、11.2、11.9、11.12、11.26～11.31 见原书。
...
![图 11.26 习题 8 的 3 阶 B 树图示](assets/scan/fig-11-26.png)
...
![图 11.27 习题 9 的 4 阶 B 树](assets/scan/fig-11-27.png)
...
![图 11.28 习题 10 的 3 阶 B 树](assets/scan/fig-11-28.png)
...
![图 11.29 2 阶 B+ 树图示](assets/scan/fig-11-29.png)
...
![图 11.30 混合型 B+ 树，内部结点阶为 4、叶结点阶为 3](assets/scan/fig-11-30.png)
...
![图 11.31 红黑树](assets/scan/fig-11-31.png)
