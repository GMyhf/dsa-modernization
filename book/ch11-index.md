...
查找 50：根上 $30<50<70$，走中间孩子；叶上直接取到 50。查找 35 到 80：先落到含 35 的叶，再沿 `↔` 扫过 50、70，直到超过 80。插入 60：中间叶暂成 `[30|50|60]`，超过 3 阶叶最多两个 key 的上限，需要分裂。为使两边都满足最小占用，可分成 `[30]` 和 `[50|60]`，并把右叶的首 key `50` 拷一份上推（B+ 的分界 key 在叶上仍保留一份；具体的左右分配取决于实现约定）。删除 70：先在叶上删，若叶下溢就向兄弟借或合并，再改内部结点的路标。
