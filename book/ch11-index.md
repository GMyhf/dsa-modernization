...
![图11.15 百货销售数据库的位图索引](assets/012aa9549249f6c8.jpg)
...
红黑树的“平衡”不是要求左右子树等高，而是用颜色约束控制最长路径：根为黑、红结点不能有红孩子、从任一结点到其后代空叶的黑结点数相同。插入先按普通二叉搜索树放入红结点，再通过换色和旋转修复红红冲突。这里不展开全部修复算法，因为本章的外存主线是 B+ 树；图11.16只用来比较“二叉、内存结点”与“多路、页结点”的不同成本模型。
