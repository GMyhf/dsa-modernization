...
# 两侧要求是字母数字或汉字。最初只收字母数字——真实撞见的三处都是数字区间
# （`0--127`、`5--8`、`7--12`），而当时没有一条自测能证明汉字那一半有用，
# 就按「没有测试撑着的能力就是负担」把它砍了，并留话说「哪天真出现『北京--上海』
# 这种写法，连同用例一起加回来」。2026-08-18 真出现了：习题答案 ch12 第 3 题写的是
# 「标记--清扫」。所以汉字连同 test_chinese_compound_with_double_dash_is_reported 一起加回来。
DOUBLE_DASH_RE = re.compile(r"[0-9A-Za-z\u4e00-\u9fff]--[0-9A-Za-z\u4e00-\u9fff]")
