...
6. **教学正确性。** 闸门管代码，不管讲法。书稿的文字是否讲清楚了，
   只有人能判断。
7. **判据比性质弱的测试。** 断言全绿只说明「被断言的那件事成立」，
   不说明「该成立的都成立」。实例（2026-09-11）：算法6.10 的还原测试原来只对
   **先根序列**，而先根序列**不唯一确定一片森林**——书稿把图 6.15 的 `ltag`/`rtag`
   两行抄反了（那张图从上到下印的是 rtag / info / ltag），还原出的是另一片森林，
   先根序列却一字不差，于是闸门一年多没红。已补上后根序列与逐点父子/兄弟断言。
   **同类风险还在别处**：凡是用「某个序列对得上」当判据的测试，都要先问一句
   「这个序列是不是唯一确定了我要验的结构」。
8. **作者代码包那一步（2026-09-18，T-079）证明得比它的输出看上去少。**
   `authorsrc.py --check` 的「54 个程序，g++ 下 38 过 16 不过」只是 `-fsyntax-only`：**没链接、没运行**，
   而且基线按工具链分开登记（g++ / clang / clang+libc++ / Apple clang，后者 2026-09-19 由 Codex 在 Mac 上登记）；
   **没有登记基线的工具链（例如换一台装了别的编译器版本的机器）只警告、不核对**。
   第 8 章的排序在 clang 下「编译得过」只因模板从没实例化——真调用就报两阶段查找错误，
   编译得过的 25 个里照样有越界（P01 `MaxHeap`、P02 冒泡都在编译得过的程序里）。
   31 条结论的正则证明的是「那句代码在包里」，**不是**「那句代码在运行时出错」——
   只有带 `harness` 的 5 条（E05、E17、R04、R12、P01/P02/P06 所在的 4 个对拍）是跑出来的。
   对拍只覆盖第 2、3、4、8 章；第 5–7、9–12 章的包代码**一行都没执行过**，
   包里那里还有多少只在包里的坑，不知道。对拍里作者代码的泄漏被有意放过（`detect_leaks=0`）。
