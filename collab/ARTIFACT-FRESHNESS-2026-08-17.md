...
## 收口追记（Claude，2026-08-18）

**建议顺序的三项全部落地**，这份清单里不再有缺闸门的黄色行：

1. 插图集离线 `--check`（D-020，2026-08-17，闸门第 10 步）；
2. 习题答案结构化绑定 —— 做成了 **R14**（D-021，同号答案或登记欠着）+ **R15**（D-022，
   书稿点名的 `类名::成员` 必须解析到 `code/` 的真实成员）。R15 是被 T-037 复查逼出来的：
   那一轮在 11 步全绿的书稿里手工抓到 4 个**不存在**的接口名；
3. 正文小节到课件页的覆盖登记 —— 做成了 **R16**（D-023）+ `collab/slide_coverage.json`。
   **这一项按本文档上面那条实测警告办了：显式登记，不做匹配器。** 首轮
   正文 165 节 → 登记到课件页 142、引子 9、有意不进课件 7、**欠着 7**（见 T-040）。

**要说清楚新增的这三条闸门证明不了什么**（与 D-014 同一条边界）：

- R14 只证明「有同号答案」，不证明答案对。T-037 复查手工改红 6 处，就是这条边界的实证。
- R15 只证明「这个名字存在于那个类」，不证明签名对、参数对、语义对。
- R16 只证明「登记了、登记的页真的在」，不证明那一页讲得对、讲得够。

课件内容那一行现在的准确说法是：**覆盖关系有了逐节的显式登记和闸门，
覆盖质量仍是人工复核项。**
