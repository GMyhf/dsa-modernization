...
| T-011 | **第 5 章二叉树**（12 条清单：代码5.1/5.2、算法5.3–5.7、代码5.8、算法5.9/5.10、代码5.11/5.12） | In progress | Codex | 人于 2026-08-12 指派。任务书见 `collab/BRIEF-T011-ch05.md`。**分工**：Codex 实现 + 断言 + 证据 + 书稿（本机 `--allow-degraded`），Claude 负责完整双构建与**泄漏/悬垂专项变异自检**——这一章是目前泄漏风险最高的（树的析构、子树删除、结点回收全是裸指针）。注意代码5.8 的结束标记被 OCR 吃掉（边界在 4058–4156 之间，需人工定并写明依据） |
