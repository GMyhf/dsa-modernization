...
| T-024 | **复查 2026-08-16 那一轮**（教学版分层 / 课件对照 / 新课件） | Backlog | Codex | 任务书 `collab/BRIEF-REVIEW-2026-08-17.md`，按风险排好了顺序。**P0 两条**：并查集换成重量权衡合并（唯一改了既有语义的地方，tie-break 是我加的口径）；`算法6.10` 此前被认领却没实现，已补但请复核是否忠实原书。**明确交出去的三条**：D-007 判不了「认领的清单真的被实现了」；R8 的判据只覆盖一半（块漂开反而变哑）；图与公式没有机器核对。生成 review 包：`python3 tools/handoff.py --from claude --to codex --base 34d94a6` |
