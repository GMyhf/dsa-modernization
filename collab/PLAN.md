...
| T-004b | 第 4 章字符串类（代码4.1 / 算法4.3 / 算法4.4 / 算法4.5） | Backlog | — | 证据已采集：`String::String(char* s)` 使书中自己的例子 `String s1 = "Hello"` 在 C++11 起编译不过；算法4.5 从返回 `String` 的函数 `return NULL`；算法4.3 的 `strcmp` 与标准库同名同签名。Codex 可认领 |
