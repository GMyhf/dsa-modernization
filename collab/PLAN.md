...
| T-025 | **每条清单绑定实现锚点与测试名**（D-007 2026-08-17 补充条款） | Backlog | 未认领 | 由 T-024 的 P0 逼出来：`ch06/general_tree` 密度 7.4、D-007 两条判据全达标，却整整少实现一条【算法6.10】——D-007 量的是单元整体扎实度，平均掉了单条清单的缺席。处方（Codex 提，已采纳）：`unit.json` 的 `listings` 从裸编号升级为「编号 → { anchor, test }」，闸门至少检查锚点在源文件里存在、测试名在 `test.cpp` 里出现。它不证明实现是对的（机器判不了），但把「认领了却没写」从静默变成报红。**这是 105 条清单 / 30 余个单元的格式迁移**，会动 `ledger.py`、`check_code.py` 和每个 `unit.json`，该单独一轮做，混进复查修复会让差异读不动 |
