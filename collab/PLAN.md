...
| T-025 | **每条清单绑定实现锚点与测试名**（D-007 2026-08-17 补充条款） | Done | Codex | 105 条清单已迁移为 `{id, anchor, test}`；`check_code.py` 检查锚点分别存在于实现文件与 `test.cpp`，`ledger.py` 兼容对象台账并保留 104/105 + 1 退场等式。迁移脚本：`tools/migrate_t025.py`；工具单测通过。 |
