...
| T-012 | **第 3 章队列**（代码3.13/3.14/3.15 + 3.3 节比较） | In progress | Codex | 人于 2026-08-12 指派。任务书 `collab/BRIEF-T012-ch03-queue.md`。**`dequeue()` 必须按 D-001 §3c 返回 `optional`**（空队列是可预期状态，不抛）。循环队列的"空/满不可区分"要按原书思路处理并写明代价。sanitizer 仍由 Claude 补跑 |
| T-013 | **第 3 章栈的其余部分**（代码3.4 链式栈、算法3.5 表达式求值、算法3.6–3.12 递归与栈空间，共 9 条） | In progress | Claude | 与 T-012 同章分工，互不碰对方代码；书稿 `book/ch03-stack.md` 按 3.1.x / 3.2+3.3 分段合并 |
