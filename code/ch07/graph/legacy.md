...
覆盖代码7.1–7.3、算法7.5–7.11；原文范围 `dsa_raw.md:5780-6591`。现代 `Graph` 采用
邻接矩阵，使 DFS/BFS、拓扑、最短路和 MST 共享同一可验证的边语义。

**代码7.4（邻接表）不在本单元**，已移交 `code/ch07/adjacency_list`。此前这里认领了
代码7.4，理由写的是「代码7.3/7.4 的矩阵和邻接表语义由 `add_edge` 的有向/无向选择承载」——
那句话不成立：代码7.4 讲的是**存储结构**，与有向/无向无关。本单元从头到尾只有邻接矩阵，
台账却把代码7.4 记成了「已现代化」。2026-08-14 修正。

## 清单映射

代码7.1/7.2 的图与边 ADT 对应 `Graph`/`Edge`；代码7.3 的相邻矩阵对应 `adjacency_`
这张 $V\times V$ 的表。算法7.5–7.11 分别为 `dfs`、`bfs`、`topological_sort`、
`dijkstra`、`floyd`、`prim`、`kruskal`。代码7.4 见 `code/ch07/adjacency_list`。
