...
class InvertedIndex:
    ...
    def add_document(self, doc_id, terms):
        if self._documents and doc_id<=self._documents[-1]:
            raise ValueError("document ids must strictly increase")
        self._documents.append(doc_id)
        for position,term in enumerate(terms):
            posting = self._terms.setdefault(term, [[],[]])
            if not posting[0] or posting[0][-1]!=doc_id:
                posting[0].append(doc_id)
                posting[1].append([])
            posting[1][-1].append(position)
    def postings(self, term):
        return list(self._terms.get(term,[[],[]])[0])
    ...
    def or_query(self, terms):
        result = []
        for term in terms:
            result = unite(result,self.postings(term))
        return result
    def not_query(self, term):
        return difference(self._documents,self.postings(term))
    def phrase_query(self, terms):
        result = []
        for doc in self.and_query(terms):
            starts = self._positions(terms[0],doc)
            if any(all(start+step in self._positions(term,doc) for step,term in enumerate(terms[1:],1)) for start in starts):
                result.append(doc)
        return result
    ...
    def postings_size(self):
        return sum(len(value[0]) for value in self._terms.values())
