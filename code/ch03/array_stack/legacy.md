...
**算法3.3**（改进的进栈操作）。原文见 `dsa_raw.md:1804`、`dsa_raw.md:1834`、`dsa_raw.md:1905`。

### 代码3.1 的 OCR 边界（T-008，2026-08-13 定）

清单在 `dsa_raw.md:1804` 开，**没有** `【代码3.1结束】`。
围栏内最后一行是 `1;`（1816）——OCR 把类末尾的 `};` 认成了 `1;`。
1819 行起是「上述定义的这个抽象数据类型并不是唯一的」这句收束正文，
1823 是新小节「3.1.2 顺序栈」。因此切片收到 **1817 行围栏结束**。
此判定不改 `dsa_raw.md`。
...
也是 2025 秋期末机考的考场资料——是代码3.2 与算法3.3的第二证人。下表每一行在 `collab/authorsrc.json` 的
findings 里都有一条带正则的结论，`tools/authorsrc.py --check` 在包的源码上逐条核对；
看包里原文：`python3 tools/authorsrc.py --listing 3.2`。
包与原书同为 2008 年 6 月，谁先谁后不可考：「包里没有」只说明**印出来的与作者的代码不一致**，不说明是排印时引入的。

| 结论 | 缺陷 | 包里 | 说明 |
| --- | --- | --- | --- |
| E05 | `int top` 与 `bool top(T&)` 重名 | 没有 | 包里取栈顶叫 `getTop(T*)`，`int top` 是 public 数据成员，**整个文件编译得过** |
| E06 | 算法3.3 的 `i` 未声明 | 包里无此代码 | 包里三份 `arrStack.h` 的 `push` 都是栈满即打印「栈满溢出」返回 `false`，没有翻倍扩容 |
| P07 | 无参构造留下未初始化成员（缺陷 3） | **也有** | 包里 `arrStack()` 只设 `top = -1` |
| E22 | 有析构、无拷贝控制（缺陷 4） | **也有** | 同原书 |

`author_diff.cpp` 在随机操作序列上对拍作者的 `arrStack` 与本单元的 `ArrayStack`：容量内逐步一致，
撞满后作者拒收、本单元按算法3.3 翻倍——包里没有 3.3，这一差异是预期内的，由断言钉住。
