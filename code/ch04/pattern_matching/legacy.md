...
1. `j - pLen + 1` 在算法4.6 与算法4.8 **两处独立印出**，写法一致；
2. 同一段代码里的回溯语句 `j = j - i + 1`（这一句是对的）说明作者用的就是
   0 起始下标——`+1` 在那里是"下一个起始位置"，在返回值里却成了偏移；
3. **原书 4.3 节开头的约定把这一点写死了**（`dsa_raw.md:3246`）：
   「P和T的第一个字符都从位置0开始。」
...
`String` 类本身（代码4.1、算法4.3–4.5）属于 4.2 节，见单元
`code/ch04/string_class`（已完成）。

## 作者代码包对照（2026-09-18）

作者代码包 `site_visit/DSCode_ZWZ200806_CPP/`——原书前言（`dsa_raw.md:195`）称之为「与本书配套的代码包」，
也是 2025 秋期末机考的考场资料——是算法4.6–4.8的第二证人。下表每一行在 `collab/authorsrc.json` 的
findings 里都有一条带正则的结论，`tools/authorsrc.py --check` 在包的源码上逐条核对；
看包里原文：`python3 tools/authorsrc.py --listing 4.8`。
包与原书同为 2008 年 6 月，谁先谁后不可考：「包里没有」只说明**印出来的与作者的代码不一致**，不说明是排印时引入的。

| 结论 | 缺陷 | 包里 | 说明 |
| --- | --- | --- | --- |
| E17 / E21 / R10 / R13 | 返回 `j - pLen + 1` | **也有** | 作者的代码本来就按 1 起始报位置，不是排印问题；`author_diff.cpp` 对拍 407 组，找到时作者恒比 0 起始位置大 1 |
| R12 | `while (i<m-1)` 下多余的 `break` | 没有 | 包里是 `while (i < m)`，`if (i == m) break` 是必需的；印成 `i < m-1` 才显得多余 |

`author_diff.cpp` 把包里的 `findNext` 与本单元的 `build_next` 逐项比对（都带 `next[i] = next[k]` 优化，逐项相等），
并断言「找到时作者恰好 +1、没找到时两边都报没找到」。本单元若退回原书的 `+ 1`，这条对拍立刻变红。
