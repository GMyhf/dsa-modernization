...
书稿   30 个文件（12 章正文 + 总目录 + 习题 + 期中复习题库 + 勘误 + 插图，加 13 套课件），17 条规则通过
成品   PDF（book/pdf/，592 页）、网页版（book/site/，双击 index.html 即可读）
...
python3 tools/build_pptx.py           # 同一份课件源，另出一份 .pptx（book/slides/pptx/）
python3 tools/check_courseware.py      # courseware/ 那套讲义+课件+视频的闸门（含真调 LibreOffice 的版面检查）
```

网页版在线可读：**<https://gmyhf.github.io/dsa-modernization/>**，
首页顶上有整本 PDF 的下载卡片（592 页 / 7.6 MB，卡片上的数字由构建器读文件得出，不是手写的）。
